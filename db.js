// Database layer for Gym Tracker PWA using IndexedDB
class GymTrackerDB {
    constructor() {
    ...
    async exportData() {
        ...
        const exerciseSets = await this.getAllExerciseSets();
    ...
    async importData(importData) {
        try {
            const { workouts, exerciseSets, exercises, settings } = importData.data;
            
            // Import workouts
            for (const workout of workouts) {
                const { id, ...workoutData } = workout;
                await this.add('workouts', workoutData);
            }
            
            // Import exercise sets
            for (const set of exerciseSets) {
                const { id, ...setData } = set;
                await this.add('exerciseSets', setData);
            }
            ...
            for (const exercise of exercises) {
                if (exercise.isCustom) {
                    ...
                    await this.add('exercises', exerciseData);
                }
            }
    ...
    // Clear all data
    async clearAllData() {
        const stores = ['workouts', 'exerciseSets', 'exercises', 'settings'];
